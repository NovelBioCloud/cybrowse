...
export default class NetworkConfigModuleFactory {
	constructor(props = {}) {
		this.props = props;
	}
	getReducer() {
		return (state, action) => {
			switch (action.type) {
			case "networkConfig.refresh":
				return {
					...state,
					time: Date.now()
				}
			case "networkConfig.reset":
				return {
					...state,
					time: 0
				}
	...
	_getDefaultCommponent() {
		class View extends Component {
			render() {
				return (
					<div>
						<div>{this.props.networkConfig.time}</div>
						<button onClick={()=>{
							this.props.refresh()
						}}>refresh</button>
					</div>
  ...
  _getActions() {
    return {
    	refresh: data => {
    		return {
    			type: 'networkConfig.refresh',
    			time: Date.now()
    		}
    	},
			reset: data => {
    		return {
    			type: 'networkConfig.reset',
    			time: 0
    		}
    	}
    }
